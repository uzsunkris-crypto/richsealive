...
// === CONFIG ===
const contractAddr = "0xb93d116A1CB1AEE03A64053542Ec966368652873"; // Your token
const pancakeRest = `https://api.pancakeswap.info/api/v2/tokens/${contractAddr}`;
const pancakeSubgraph = "https://bsc.streamingfast.io/subgraphs/name/pancakeswap/exchange-v2";
...
async function loadHistorical() {
  const query = `
    {
      tokenDayDatas(first: 30, orderBy: date, orderDirection: asc, where: { token: "${contractAddr.toLowerCase()}" }) {
        date
        priceUSD
      }
    }
  ...
  try {
    const res = await fetch(pancakeSubgraph, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ query })
    });
    const json = await res.json();
    const data = json.data.tokenDayDatas;
    ...
    // Convert to candlestick (using priceUSD as open/close/high/low since Subgraph has only daily price)
...
async function loadLivePrice() {
  try {
    const res = await fetch(pancakeRest);
    const json = await res.json();
    if (json.data) {
      const price = Number(json.data.price);
      priceEl.textContent = `$${price.toFixed(6)}`;

      // Push live price as new candle (fake OHLC with same value)
    ...
    console.error("Error loading live price:", err);
  }
}
...
loadHistorical();
loadLivePrice();
setInterval(loadLivePrice, 10000); // Refresh every 10s

// === Responsive Resize ===
window.addEventListener("resize", () => {
  chart.resize(document.getElementById("chart").clientWidth, 480);
});
